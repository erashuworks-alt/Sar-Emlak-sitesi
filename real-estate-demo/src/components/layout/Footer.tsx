              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <Link
                    key={item.label}
                    href={item.href}
                    className="px-4 py-2 min-h-[44px] inline-flex items-center justify-center text-sm text-slate-600 dark:text-slate-300 bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#D4AF37]/30 rounded-full hover:border-brand-400 hover:text-brand-600 dark:hover:text-[#F4E4A6] hover:bg-brand-50 dark:hover:bg-[#D4AF37]/10 transition-all duration-200 hover:-translate-y-0.5 shadow-[0_2px_10px_rgba(0,0,0,0.3)]"
                  >
                    {item.label}
                  </Link>
                ))}
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
              ...
              <div className="flex gap-4">
                {footer.socialLinks.filter((s: any) => s.isActive).map((social: any) => (
                  <a key={social.id} href={social.url} target="_blank" rel="noopener noreferrer" 
                    className="p-3 w-11 h-11 flex items-center justify-center rounded-full glass hover:bg-slate-100 dark:hover:bg-[#111827] transition-colors text-slate-600 dark:text-[#D4AF37] border dark:border-[#D4AF37]/30 shadow-[0_4px_15px_rgba(212,175,55,0.15)]">
            ...
            <div>
              ...
              <ul className="flex flex-col gap-3">
                {footer.quickLinks.filter((l: any) => l.isActive).map((link: any) => (
                  <li key={link.id}>
                    <Link href={link.href} className="text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-[#F4E4A6] transition-colors block py-2 min-h-[44px] flex items-center">
                      {link.title}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="flex flex-col gap-3">
                <li><Link href="/properties?category=apartment" className="text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-[#F4E4A6] transition-colors block py-2 min-h-[44px] flex items-center">Daire</Link></li>
                <li><Link href="/properties?category=villa" className="text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-[#F4E4A6] transition-colors block py-2 min-h-[44px] flex items-center">Villa</Link></li>
                <li><Link href="/properties?category=office" className="text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-[#F4E4A6] transition-colors block py-2 min-h-[44px] flex items-center">Ofis</Link></li>
                <li><Link href="/properties?category=land" className="text-slate-500 hover:text-brand-600 dark:text-slate-400 dark:hover:text-[#F4E4A6] transition-colors block py-2 min-h-[44px] flex items-center">Arsa</Link></li>
              </ul>
            </div>
