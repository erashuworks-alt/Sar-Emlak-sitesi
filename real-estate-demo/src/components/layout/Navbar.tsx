        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex items-center justify-between">
            ...
            <Link href="/" className="flex items-center gap-2 text-2xl font-extrabold tracking-tighter text-[#D4AF37] hover:text-[#F4E4A6] transition-all duration-300 drop-shadow-[0_2px_15px_rgba(212,175,55,0.6)] group">
              ...
              <span className="bg-red-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full select-none shrink-0 shadow-sm border border-red-400">DEMO</span>
            </Link>
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed inset-y-0 left-0 w-[300px] max-w-[85vw] bg-white dark:bg-[#0B1120] border-r border-[#D4AF37]/40 shadow-[20px_0_50px_rgba(212,175,55,0.2)] z-[101] flex flex-col md:hidden"
            >
              <div className="flex items-center justify-between p-6 border-b border-[#D4AF37]/20">
                <span className="text-lg font-bold text-[#D4AF37]">Menü</span>
                <button 
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="p-2.5 rounded-xl bg-[#D4AF37]/10 text-[#D4AF37] hover:bg-[#D4AF37]/25 transition-all w-11 h-11 flex items-center justify-center"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-2">
                {header.navigation.filter((l: any) => l.isActive).sort((a: any, b: any) => a.order - b.order).map((link: any) => (
                  <Link key={link.id} href={link.href}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="text-base font-bold py-3.5 px-5 rounded-xl hover:bg-[#D4AF37]/15 dark:hover:bg-[#D4AF37]/20 text-slate-800 dark:text-[#F9FAFB] hover:text-[#D4AF37] dark:hover:text-[#F4E4A6] transition-all duration-200 min-h-[44px] flex items-center">
                    {link.title}
                  </Link>
                ))}
              </div>
              <div className="p-6 border-t border-slate-200 dark:border-[#D4AF37]/30 flex flex-col gap-4 bg-slate-50 dark:bg-[#050816]">
                <button 
                  onClick={() => {
                    toggleDarkMode();
                  }}
                  className="flex items-center justify-center gap-3 p-3.5 min-h-[44px] rounded-xl bg-[#D4AF37]/10 dark:bg-[#D4AF37]/15 text-slate-800 dark:text-[#D4AF37] font-bold border border-[#D4AF37]/40 shadow-[0_4px_15px_rgba(212,175,55,0.25)] hover:bg-[#D4AF37]/20 transition-all duration-200"
                >
                  {isDark ? <><Sun className="w-5 h-5 text-[#F4E4A6]" /> Açık Mod</> : <><Moon className="w-5 h-5 text-[#D4AF37]" /> Koyu Mod</>}
                </button>
                {header.showCTA && (
                  <Link href={header.ctaLink} onClick={() => setIsMobileMenuOpen(false)} className="w-full">
                    <Button className="w-full rounded-xl min-h-[44px] py-3 text-base font-extrabold bg-gradient-to-r from-[#D4AF37] via-[#C9A227] to-[#D4AF37] text-[#050816] hover:from-[#F4E4A6] hover:via-[#D4AF37] hover:to-[#C9A227] transition-all duration-500 shadow-[0_6px_25px_rgba(212,175,55,0.6)] border border-[#F4E4A6]/50 tracking-wide">
                      {header.ctaText}
                    </Button>
                  </Link>
                )}
