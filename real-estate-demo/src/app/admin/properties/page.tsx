                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1 justify-end flex-nowrap">
                          <button onClick={() => toggleFeatured(p.id)} title={p.isFeatured ? "Öne çıkarmayı kaldır" : "Öne çıkar"}
                            className={`p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg transition-colors ${p.isFeatured ? "text-yellow-500 bg-yellow-50 dark:bg-yellow-900/20" : "text-slate-400 hover:bg-slate-100"}`}>
                            <Star className={`w-4 h-4 ${p.isFeatured ? "fill-yellow-500" : ""}`} />
                          </button>
                          {p.status === "pending" && (
                            <button onClick={() => setStatus(p.id, "published")} title="Onayla"
                              className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-slate-400 hover:text-emerald-600 transition-colors">
                              <CheckCircle className="w-4 h-4" />
                            </button>
                          )}
                          <button onClick={() => openEdit(p)} title="Düzenle"
                            className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg hover:bg-brand-50 dark:hover:bg-brand-900/20 text-slate-400 hover:text-brand-600 transition-colors">
                            <Pencil className="w-4 h-4" />
                          </button>
                          <button onClick={() => togglePublish(p.id)} title={p.status === "published" ? "Yayından Kaldır" : "Yayınla"}
                            className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-brand-600 transition-colors">
                            {p.status === "published" ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                          </button>
                          <button onClick={() => setDeleteTarget(p.id)} title="Sil"
                            className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-600 transition-colors">
