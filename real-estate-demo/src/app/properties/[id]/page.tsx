...
export default function PropertyDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pt-20">
      {/* Image Gallery */}
      <div className="relative h-[40vh] md:h-[60vh] min-h-[280px] md:min-h-[500px] w-full bg-slate-900 group">
        <Image 
        ...
        {property.images.length > 1 && (
          <>
            <button onClick={prevImage} className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 p-2.5 md:p-3 rounded-full glass hover:bg-white/90 transition-all opacity-75 md:opacity-0 md:group-hover:opacity-100 z-10">
              <ChevronLeft className="w-5 h-5 md:w-6 md:h-6 text-slate-900" />
            </button>
            <button onClick={nextImage} className="absolute right-4 md:right-6 top-1/2 -translate-y-1/2 p-2.5 md:p-3 rounded-full glass hover:bg-white/90 transition-all opacity-75 md:opacity-0 md:group-hover:opacity-100 z-10">
        ...
        {/* Gallery Indicator */}
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 glass px-4 py-2 rounded-full text-xs md:text-sm font-medium text-slate-900">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center py-2 border-b border-slate-100 dark:border-slate-800 gap-4 flex-wrap">
                    <span className="text-sm font-semibold text-slate-500 dark:text-slate-400">{item.label}</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-white text-right break-words max-w-[60%]">{item.value}</span>
                  </div>
                ))}
              </div>
            </div>
