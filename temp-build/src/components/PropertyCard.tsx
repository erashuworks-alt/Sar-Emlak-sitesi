    ...
    >
      ...
      <div className="p-8 flex-1 flex flex-col justify-between bg-white dark:bg-[#111827]">
        <div>
          ...
          <div className="grid grid-cols-3 gap-2 py-3.5 px-4 rounded-2xl bg-slate-50 dark:bg-[#0B1120] border border-slate-100 dark:border-[#D4AF37]/20 mb-8 text-xs font-bold text-slate-700 dark:text-slate-300 shadow-inner">
            <div className="flex items-center justify-center gap-2 border-r border-slate-200 dark:border-slate-800/80">
              <BedDouble className="w-4 h-4 text-brand-600 dark:text-[#D4AF37]" />
              <span>{roomCount}</span>
            </div>
            <div className="flex items-center justify-center gap-2 border-r border-slate-200 dark:border-slate-800/80">
              <Bath className="w-4 h-4 text-brand-600 dark:text-[#D4AF37]" />
              <span>{bathrooms}</span>
            </div>
            <div className="flex items-center justify-center gap-2">
              <Square className="w-4 h-4 text-brand-600 dark:text-[#D4AF37]" />
              <span>{squareMeters} m²</span>
            </div>
          </div>
        </div>

        <div className="mt-auto flex items-end justify-between pt-6 border-t border-slate-100 dark:border-[#D4AF37]/20">
          <div>
            ...
            <div className="text-2xl md:text-3xl font-extrabold tracking-tight text-brand-600 dark:text-[#D4AF37] bg-gradient-to-r dark:from-[#D4AF37] dark:via-[#F4E4A6] dark:to-[#C9A227] dark:bg-clip-text dark:text-transparent">
              {new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY', maximumFractionDigits: 0 }).format(price)}
            </div>
          </div>
          <Link
            href={`/properties/${id}`}
            className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-slate-900 dark:bg-[#D4AF37]/10 hover:dark:bg-[#D4AF37]/20 text-white dark:text-[#F4E4A6] text-xs font-bold uppercase tracking-wider transition-all border border-transparent dark:border-[#D4AF37]/30 hover:gap-3 shadow-[0_4px_15px_rgba(0,0,0,0.2)] dark:shadow-[0_4px_15px_rgba(212,175,55,0.15)]"
          >
