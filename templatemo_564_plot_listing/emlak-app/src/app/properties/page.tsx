    ...
    <div className="flex flex-wrap gap-2 mb-4">
      {badges.map(({ key, label }) => (
        <motion.button key={key} layout initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          className="flex items-center gap-1.5 px-4 min-h-[44px] bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-400 text-xs font-medium rounded-full border border-brand-200 dark:border-brand-800 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 hover:border-red-200 transition-all">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              className="min-h-[44px] pl-9 pr-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-brand-500 outline-none appearance-none">
            ...
            <div className="flex bg-white dark:bg-slate-800 rounded-xl p-1 shadow-sm border border-slate-200 dark:border-slate-700">
              <button onClick={() => setViewMode("grid")} className={`p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg transition-colors ${viewMode === "grid" ? "bg-brand-50 dark:bg-brand-900/30 text-brand-600" : "text-slate-500 dark:text-slate-400"}`}>
                <Grid className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode("list")} className={`p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg transition-colors ${viewMode === "list" ? "bg-brand-50 dark:bg-brand-900/30 text-brand-600" : "text-slate-500 dark:text-slate-400"}`}>
                <List className="w-4 h-4" />
              </button>
            </div>
            {/* Mobile filter button */}
            <button onClick={() => setDrawerOpen(true)}
              className="lg:hidden flex items-center justify-center gap-2 min-h-[44px] px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-medium text-slate-700 dark:text-slate-300">
